...
public final class GeneratedConfigLoader implements ConfigLoader {
    public static final FieldReference<List<Integer>> listField = FieldReference.builder(value -> ListField.listField = value, () -> ListField.listField, new GenericType(List.class, new GenericType(Integer.class)), "LIST_FIELD").doesSync(true).validator(ValidatorReference.<List<Integer>>builder().messageProvider(ignored -> "Value for 'LIST_FIELD' is invalid. Resetting to default value: 'List.of(1, 2, 3, 4, 5)'").fallback(true).defaultSupplier(() -> List.of(1, 2, 3, 4, 5)).build()).build();
