		...
		if (!onSetMethod.isEmpty()) {
			...
			if (!processingEnv.getTypeUtils().isAssignable(actualReturnType, expectedReturnType)) {
				processingEnv.getMessager()
						.printError("Return type of method '%s' must be void, but found '%s'".formatted(onSetMethod, actualReturnType), onSetReference.methodElement());
				return null;
			}
			...
			if (parameterTypes.size() != 2) {
				...
				processingEnv.getMessager()
						.printError("Method '%s' must have exactly two parameters, but found %d".formatted(onSetMethod, parameterTypes.size()), problemElement);
				return null;
			}
			...
			if (!processingEnv.getTypeUtils().isSameType(firstParamterType, expectedParameterType)) {
				...
				processingEnv.getMessager()
						.printError("Method '%s''s first parameter must be of type '%s', but found '%s'".formatted(onSetMethod, expectedParameterType, firstParamterType), problemElement);
				return null;
			}
			TypeMirror secondParameterType = parameterTypes.get(1);
			if (!processingEnv.getTypeUtils()
					.isSameType(secondParameterType, processingEnv.getTypeUtils().getPrimitiveType(TypeKind.BOOLEAN))) {
				VariableElement problemElement = onSetReference.methodElement().getParameters().get(1);
				processingEnv.getMessager()
						.printError("Method '%s''s second parameter must be of type 'boolean', but found '%s'".formatted(onSetMethod, secondParameterType), problemElement);
				return null;
			}
		}
